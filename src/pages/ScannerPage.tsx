...
export default function ScannerPage({ user }: Props) {
  ...
  return (
    <div className="p-6 px-10 max-w-[880px] mx-auto">
      <h1 className="text-[28px] font-black mb-1 text-gray-900">
        Scanner un meuble
      </h1>
        ...
        <div>
          ...
          >
            <div
              ...
              style={{ opacity: dragging ? 0.8 : 0.3 }}
            >
              <svg
                width="40"
                height="40"
            ...
            <p className="text-[13px] text-gray-400 leading-relaxed">
              Prenez 20–40 photos en tournant autour du meuble
              <br />
              Format JPG, PNG, WebP · Max 10 Mo par photo
            </p>
            <input
              ref={fileInputRef}
              type="file"
      ...
      {step === 1 && (
        <div>
          <div className="grid grid-cols-2 gap-5 mb-5">
            <div>
              ...
              <select
                value={meta.category}
                onChange={(e) =>
                  setMeta({ ...meta, category: e.target.value as any })
                }
          ...
          <div className="grid grid-cols-2 gap-5 mb-5">
            <div>
              <label className="label-base">Largeur (cm)</label>
              <input
                type="number"
                value={meta.w}
                onChange={(e) => setMeta({ ...meta, w: e.target.value })}
                className="input-base"
              />
            </div>
            <div>
              <label className="label-base">Hauteur (cm)</label>
              <input
                type="number"
                value={meta.h}
                onChange={(e) => setMeta({ ...meta, h: e.target.value })}
                className="input-base"
              />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-5 mb-5">
            <div>
              <label className="label-base">Profondeur (cm)</label>
              <input
                type="number"
                value={meta.d}
                onChange={(e) => setMeta({ ...meta, d: e.target.value })}
                className="input-base"
              />
            </div>
            <div>
              <label className="label-base">Quantité disponible</label>
              <input
                type="number"
                value={meta.qty}
                onChange={(e) => setMeta({ ...meta, qty: e.target.value })}
                className="input-base"
              />
            </div>
          </div>
          <div className="mb-5">
            <label className="label-base">Couleur / Matière</label>
            <input
              value={meta.material}
                ...
                return (
                  <button
                    ...
                    className={`py-4 px-3 rounded-xl text-center text-[13px] font-semibold cursor-pointer transition-all ${
                      active
                        ? "bg-brand-light border-2 border-brand/30 text-brand"
                        : "bg-surface-alt border border-border text-gray-400"
                    }`}
                  >
          ...
          <div className="mb-7">
            <label className="label-base">Notes / Observations</label>
            <textarea
              value={meta.notes}
      ...
      {step === 2 && (
        <div className="card p-10 text-center">
          <div className="w-[120px] h-[120px] mx-auto mb-8 relative">
            ...
            {/* Arc extérieur — tourne dans le sens horaire */}
            <svg
              className="absolute inset-0 w-full h-full"
              viewBox="0 0 120 120"
            ...
            {/* Arc intérieur — tourne en sens inverse */}
            <svg
              className="absolute inset-0 w-full h-full"
              viewBox="0 0 120 120"
          ...
          <div className="max-w-[600px] mx-auto text-left">
            {[
              {
                label: "Détection des points clés",
                val: progress.keys,
                color: "#004AAD",
              },
              {
                label: "Alignement des images",
                val: progress.align,
                color: "#059669",
              },
              {
                label: "Génération du maillage",
                val: progress.mesh,
                color: "#D97706",
              },
              {
                label: "Application des textures",
                val: progress.tex,
                color: "#9333EA",
              },
            ].map((bar, i) => (
              <div key={i} className="mb-4">
                <div className="flex justify-between text-xs mb-1.5">
                  <span
                    className={bar.val > 0 ? "text-gray-600" : "text-gray-400"}
                  >
                    {bar.label}
                  </span>
          ...
          <div className="card overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-border">
              <span className="text-sm font-bold text-gray-900">
                Modèle 3D — {result.name}
              </span>
              <div className="flex gap-1.5">
                {[RotateCw, Search, Target, Package].map((Icon, i) => (
                  <div
                    key={i}
                    className={`w-8 h-8 rounded-lg flex items-center justify-center cursor-pointer ${i === 0 ? "bg-brand text-white" : "bg-gray-50 text-gray-400 hover:bg-gray-100"}`}
                  >
                    <Icon size={16} />
                  </div>
                ))}
            ...
            <div className="h-[360px] bg-[#f0f2f5]">
            ...
            <div className="flex justify-between px-5 py-2.5 border-t border-border text-[11px] text-gray-400">
              ...
              <span className="text-brand font-medium">
                ✓ {result.polygons.toLocaleString("fr")} polygones
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4">
            <div className="card p-6">
              ...
              {[
                ...
                [
                  "Dimensions",
                  `${result.dimensions.w} × ${result.dimensions.d} × ${result.dimensions.h} cm`,
                ],
                ...
                <div
                  key={i}
                  className={`flex justify-between items-center py-2.5 ${i < 6 ? "border-b border-border" : ""}`}
                >
                  ...
                  {label === "État" ? (
                    ...
                    <span
                      className={`text-sm font-semibold text-gray-900 ${val === result.assetId ? "font-mono" : ""}`}
                    >
                      {val}
                    </span>
                  )}
            ...
            <div className="card p-6 text-center">
              ...
              <div className="text-[52px] font-black text-brand font-mono leading-none">
                {result.price} <span className="text-[32px]">€</span>
              </div>
              <div className="text-xs text-gray-400 mt-2">
                prix unitaire recommandé
              </div>
              <div className="text-[13px] text-gray-500 mt-2">
                Min {result.priceMin} € — Max {result.priceMax} €
              </div>
              <div className="text-[11px] text-gray-400 mt-3">
                Basé sur {result.salesCount} ventes récentes · Neuf à ~
                {result.priceRef.toLocaleString("fr")} €
              </div>
            </div>
